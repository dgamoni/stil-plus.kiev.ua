...
function miss_header_sociable($args = array()) {
	...
	$out = '';
	$out .= '<div class="'.$args->container_class.'">';
	$out .= miss_social_shortcuts();
	$out .= '</div>';

	if ( $args->echo )
		echo apply_atomic_shortcode( 'sociable', $out );
	else
		return $out;
...
function miss_logo($args = array()) {
	...
	// if( $display_logo ) {
	// 	$logo_url = miss_get_setting( 'logo_url' );
		
	// 	if( $logo_url )
	// 		$logo = '<img src="' . esc_url( miss_get_setting( 'logo_url' ) ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '"  />';
			
	// 	elseif( !$logo_url ) {
	// 		$active_skin = apply_filters( 'miss_active_skin', get_option( MISS_ACTIVE_SKIN ) );
	// 		$color_scheme = ( !empty( $active_skin['style_variations'] ) ) ? str_replace( '.css','', $active_skin['style_variations'] ) : '';
			
	// 		if( @is_file( THEME_ASSETS_DIR . '/styles/' . $color_scheme . '/logo.png' ) )
	// 			$logo = '<img src="'  . esc_url( THEME_ASSETS ) . '/styles/' . $color_scheme . '/logo.png" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" />';
				
	// 		else
	// 			$logo = '<img src="'  . esc_url( THEME_ASSETS ) . '/images/production/logo.png" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" />';
	// 	}
		
	// } elseif ( !$display_logo ) {
	// 	$logo = Array (
	// 		'input' => get_bloginfo( 'name' ),
	// 		'out' => ''
	// 	);
	// 	$logo['array'] = explode("\x20", $logo['input']);
	// 	foreach ($logo['array'] as $part) {
	// 		$logo['out'] .= '<span>'.$part.'</span>';
	// 	}
	// 	$logo = $logo['out'];
...
function miss_recent_tweets() {
      ...
      foreach ( $results as $key => $tweet ) {
        if ( $key == "errors" ) {
        	if ( isset($tweet) && isset($tweet[0]) && isset($tweet[0]['message']) ) {
	            $out .= '<li>' . $tweet[0]['message'] . '</li>';
	        }
        } else {
            $out .= '<li>';
            $out .= '<i class="im-icon-twitter pull-left"></i><em>' . sprintf( esc_attr__( ' %1$s ago ', MISS_TEXTDOMAIN ), miss_relative_time(strtotime( $tweet['created_at'] ) ) ) . '</em><a class="target_blank" target="_BLANK" href="http://twitter.com/1/status/' . $tweet['id_str'] . '" title="' . sprintf( esc_attr__( '%1$s&nbsp;ago', MISS_TEXTDOMAIN ), miss_relative_time(strtotime( $tweet['created_at'] ) ) ) . '">' . miss_filter_tweet( $tweet['text'] ) . '</a>';
            $out .= '</li>';
        }
      }
    } else {
	...
	$out = '
	<section class="before_footer_twitter">
	<div class="container">
...
function miss_post_nav() {
	...
	echo '<div class="post_nav_module">';
	echo '<div class="previous_post">';
	previous_post_link( '%link', '<i class="im-icon-arrow-left"></i> ' . $prev . ' <br /><span>%title</span>' );
	echo '</div>';
	echo $index;
	echo '<div class="next_post">';
	next_post_link( '%link', $next.' <i class="im-icon-arrow-right-2"></i> <br /><span>%title</span>' );
	echo '</div>';
	echo '</div><!-- #nav-below -->';
...
function miss_social_shortcuts() {
    ...
    if( $sociable['keys'] != '#' ) {
        ...
        foreach ( $sociable_keys as $key ) {
            if( $key != '#' ) {
                $sociable_link = ( !empty( $sociable[$key]['link'] ) ) ? $sociable[$key]['link'] : '#';
                $sociables .= '<a href="' . $sociable_link . '" class="social_icon"><i class="' . $sociable[$key]['icon'] . '"></i></a>';
            }
        }
    }
    return $sociables;
...
function miss_partners_shortcuts( $limit = 999, $start = 0 ) {
        ...
        if( ($partners['keys'] != '#') and ($partners != '') ) {
			...
			$out .= '<li class="embedded"><div class="row-fluid carousel-wrap">';
			foreach ($partners as $key => $value) {
				if ( $span_walk >= $limit ) {
					break;
				}
				if ( $counter_item > $span_walk and ($spans_in_row * $row_walk) == $span_walk ) {
					$out .= '</div><!-- /.row-fluid --></li> <li class="embedded"><div class="row-fluid">';
				} else {
					$out .= '';
				}
				...
				if (isset($partners[$key]['custom'])){
					$out .= '<div class="partner_item" style="width: ' . $span_width . '%;">';
					$out .= '<a class="partner_link" href="' . esc_url( $partners[$key]['link'] ) . '"><img src="' . esc_url( $partners[$key]['custom'] ) . '" alt="' . esc_url( $partners[$key]['link'] ) . '" /></a>';
					$out .= '</div>';
				}
			}
			$out .= '</div></li>';
...
if ( !function_exists( 'miss_post_icon' ) ) :
	function miss_post_icon ( $args = array() )
		...
		$out = '';
		$out .= '<div class="date_likes_holder">';
		$out .= '<div class="post_icon">';
		$out .= '<i class="' . $post_icon . '"></i>';
		$out .= '</div>';
//		$out .= miss_likes( false );
		$out .= '</div>';
		if($echo == false){
...
if ( !function_exists('miss_add_header') ) {
	...
	function miss_add_header( $args = array( 'echo' => 'true' ) ){
		...
		$defaults = Array(
			...
			'subs' => miss_get_setting('sticky_hide_subs') ? miss_get_setting('sticky_hide_subs') : 'true',
			'offset' => miss_get_setting('sticky_offset') ? miss_get_setting('sticky_offset') : '200',
		...
		$site_menu = '';
		$site_menu .= '<header class="' . $class . '" data-opacity="' . $defaults['opacity'] . '" data-subs="' . $defaults['subs'] . '" data-offset="' . $defaults['offset'] . '">';
		...
		if ( has_nav_menu('primary-menu') ) {
			...
			$menu_class = 'nav-collapse main_menu';
			...
			$menu_args = array(
				...
				'menu_class' => 'nav primary hover',
				'walker' => new Megamenu_Walker_Nav_Menu, 
				'echo' => false, 
			);
			$site_menu .= wp_nav_menu( $menu_args );
		}
...
if ( !function_exists('miss_add_after_header') ) {
	...
	function miss_add_after_header( $args = array( 'echo' => 'true' ) ){
		...
		if( is_front_page() || ( !miss_page_title() && !miss_breadcrumbs() ) || ( ( miss_get_setting( 'disable_page_caption' ) && in_array( 'true', miss_get_setting( 'disable_page_caption' ) ) ) && ( miss_get_setting( 'disable_breadcrumbs' ) && in_array( 'true', miss_get_setting( 'disable_breadcrumbs' ) ) ) ) ){
			...
			$out .= '<section class="page_caption">';
			$out .= '<div class="container">';
			$out .= '<div class="row-fluid">';
			...
			$out .= '</div><!-- /.row-fluid -->';
			$out .= '</div><!-- /.container -->';
			$out .= '</section><!-- /.region page_caption-->';
		}
		...
		if ( !is_search() || !is_archive() || !is_tax() || !is_category() ) {
			if( isset($post_obj->ID) ){
				...
				/*
				elseif ( ( has_post_thumbnail( $post_obj->ID ) && !is_single() && !is_front_page() ) ) {
					$thumb = 		wp_get_attachment_image_src( get_post_thumbnail_id( $post_obj->ID ), 'large' );
					$out .= '	<section class="after_header">';
					$out .= '		<div class="container">';
					$out .= '		<div class="row-fluid">';
					$out .= '		<div class="span12">';
					$out .= '			<img class="article_header_img" src="' . $thumb[0] . '" />';
					$out .= '		</div><!-- /.-->';
					$out .= '		</div><!-- /.-->';
					$out .= '		</div><!-- /.-->';
					$out .= '	</section><!-- /.region after_header-->';
...
function miss_partners( $args = array( 'echo' => 'true' ) ){
	...
	if ( ( miss_get_setting('disable_partners_section') == 'display_all' ) || ( miss_get_setting('disable_partners_section') == 'only_front_page' && is_front_page() ) ) {
		$out .= '	<section class="partners_section">';
			if( miss_partners_shortcuts() != false ){
				$out .= '		<div class="container partners">';
				$out .= '			<div class="row-fluid">';
				$out .= '				<div class="span12">';
				$out .= '					<div class="blog_header">';
				$out .= '						<h4 class="pull-left caption">';
				$out .=							miss_get_setting('partners_caption');
				$out .= '						</h4>';
				$out .= '						<h6 class="pull-left tagline">';
				$out .=  							miss_get_setting('partners_tagline');
				$out .= '						</h6>';
