missAdmin.icons = function() {
	...
	jQuery(document).ready(function($) {
		...
		function imShowIconsList(elToggle) {


			// missAdmin.ajaxSubmit('?miss_get_icons_ajax=true');
			var default_value = elToggle.siblings('input').val();
		...
		jQuery('.im-toggle-icons').toggle(function() {
				var elToggle = jQuery(this);
				imShowIconsList(elToggle);
			// if ( im_visible_icon_selector == 0) {
				...
				elToggle.parent().parent().parent().find('.im-filter-icons-container').slideDown('slow');
				return false;
		}, function() {
				var elToggle = jQuery(this);
				...
				im_visible_icon_selector = 0;
				search_icon = 0;
				elToggle.parent().parent().parent().find('.im-filter-icons-container').slideUp('slow');
		...
		jQuery('.page-composer-icon-filter')
		  .on( 'change', function () {
		    var filter = jQuery(this).val(),
		    	list = jQuery(this).parent().parent().parent().parent().find('.im-font-icons-wrapper');
