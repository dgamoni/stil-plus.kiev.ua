...
class Megamenu_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	function std_menu_item( &$output, $args, $item, $depth ) {
		...
		$item_icon = ( get_post_meta( $item->ID, '_miss_disable_icon', true) ) ? '' : '<i class="' . $item->icon . '"></i> ';
	...
	function pages_gallery_dropdown( &$output, $args, $item, $depth ) {
		...
		$columns = get_post_meta( $item->ID, '_miss_submenu_columns', true ) ? get_post_meta( $item->ID, '_miss_submenu_columns', true ) : 2;
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n" . $indent . '<ul class="dropdown-menu' . $submenu_classes . '">' . "\n";
	}
